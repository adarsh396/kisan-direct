  ...
  return (
    <div className="py-8">
      ...
      <div className="bg-emerald-600 rounded-3xl p-10 mb-10 text-white shadow-lg flex flex-col items-center text-center">
        <h1 className="text-4xl md:text-5xl font-bold mb-8 tracking-tight">Fresh from the Farm</h1>
        <div className="relative w-full max-w-xl text-gray-900">
          <input
            type="text"
          ...
          />
          <Search className="absolute left-4 top-4 text-gray-400 mt-1" size={24} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredCrops.map((crop) => (
            <div key={crop._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300 flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center text-sm text-gray-500 mb-4 flex-grow">
                  <MapPin size={16} className="mr-1 text-amber-500" /> {crop.location}
                </div>
                <button
                  onClick={() => contactOnWhatsApp(crop.farmerPhone, crop.title)}
                  className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl hover:bg-emerald-700 transition-all shadow-md active:scale-95 mt-auto"
                >
                  Contact on WhatsApp
                </button>
              </div>
